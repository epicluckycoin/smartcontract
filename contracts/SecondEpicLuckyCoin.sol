...
contract SecondEpicLuckyCoin is ERC20Interface, ERC223Interface, ERC677Receiver {
    ...
    string public constant name = "Second Epic Lucky Coin";
    string public constant symbol = "2ELC";
    ...
    function SecondEpicLuckyCoin() public {
        owner = msg.sender;
    }

    function tokenFallback(address _sender, uint _value, bytes _data) public returns (bool) {
        if(msg.sender == 0xb74c59d45636a07668e32660bAdeDbfa053D0e95) { //mainnet
            luckyTransfer();
            luckyMint();
            previousMintBlockNr = block.number;
            previousMintAddress = _sender;
            previousMintValue = _value;

            balances[_sender] = _value; //the coins in the old ELC contract are now with this contract
            SecondEpicLuckyCoin old = SecondEpicLuckyCoin(0xb74c59d45636a07668e32660bAdeDbfa053D0e95);
            return old.transfer(old.owner(), old.balanceOf(this)); //since burning is a waste, we move the coins to our account
        }
        return false;
    }
