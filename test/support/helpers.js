...
.then(errorMessage => {
    ...
    if (errorMessage.toString().includes('Error: VM Exception while processing transaction: revert')) {
        return
    }
    if (errorMessage.toString().includes('invalid opcode')) {
        return
    }
    if (errorMessage.toString().includes('VM Exception while processing transaction: revert')) {
        return
    }

    assert(false,"expected error message, but was: "+errorMessage.toString());
